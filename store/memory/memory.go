...
func (m *memoryStore) get(prefix, key string) (*store.Record, error) {
	...
	if !storedRecord.expiresAt.IsZero() {
		newRecord.TTL = time.Until(storedRecord.expiresAt)
		newRecord.Expiry = storedRecord.expiresAt
	}
...
func (m *memoryStore) Write(r *store.Record, opts ...store.WriteOption) error {
	writeOpts := new(store.WriteOptions)
	for _, o := range opts {
		o(writeOpts)
	}
	...
	if len(opts) > 0 {
		...
		if !writeOpts.Expiry.IsZero() {
			newRecord.TTL = time.Until(writeOpts.Expiry)
			newRecord.Expiry = writeOpts.Expiry
		}

		// TTL
		if writeOpts.TTL != 0 {
			newRecord.TTL = writeOpts.TTL
			newRecord.Expiry = time.Now().Add(writeOpts.TTL)
		}
